//$ bin/sessionjc -cp tests/classes/ tests/src/nbody/Worker.sj -d tests/classes/
//$ bin/sessionj -cp tests/classes/ nbody.Worker false 4443 localhost 4444 1
//$ bin/sessionj -cp tests/classes/ nbody.Worker false 4442 localhost 4443 1
/**
* 
* @author Andi
*@modified Raymond
...
public class Worker
{
  ...
  {	
	...
	{			
	  ...
	  s_r.outwhile(s_l.inwhile())
	  {			
	    //toSent=ParticleToDouble(current);
	   //s_r.send(toSent);
       s_r.send(current);
	    
	     computeForces(particles, current, pvs);
	    //toReceive=(double[][]) s_l.receive();
	    //current=DoubleToParticle(toReceive);
  ...
  public static double[][] ParticleToDouble(Particle[] particles)
  {
   ...
   // System.out.println("Auta ta Particles pira: " + Arrays.toString(particles));
    for(int i = 0; i < particles.length; i++)
    {
 ...
 //   System.out.println("Auton ton double[][] epestrepsa ");
  //  for(int i=0; i< particles.length ; i++)
  //    System.out.println(partDouble[i][0]+" "+partDouble[i][1]+" "+partDouble[i][2]+" ");
    return partDouble;
    
  }
   public static Particle[] DoubleToParticle(double[][] partDouble)
  {
  //  System.out.println("Auton ton double[][] pira ");
 //   for(int i=0; i< particles.length ; i++)
 //     System.out.println(partDouble[i][0]+" "+partDouble[i][1]+" "+partDouble[i][2]+" ");
 ...
 //   System.out.println("Auta ta Particles epestrepsa: " + Arrays.toString(particles));
    return particles;  
  }
}
