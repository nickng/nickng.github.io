//$ sessionjc -cp lib -d lib src/jacobi/old/WorkerWest.sj
//$ sessionj  -cp lib:lib/jyaml.jar jacobi.old.WorkerWest mesh.yaml west
    ...
    public void run(int abovePort, String rightHost, int rightPort, String belowHost, int belowPort) {
        ...
        try (aboveNode) {
            ...
            try (above, right, below) {
                ...
                System.err.println("[WorkerWest@{above,right,below}] Non-dual behaviour: "+ise);

            } catch (SJIOException ioe) {

                System.err.println("[WorkerWest@{above,right,below}] Communication error: "+ioe);

            } catch (ClassNotFoundException cnfe) {

                System.err.println("[WorkerWest@{above,right,below}] Class not found: "+cnfe);

            } finally {} // Sockets

        } catch (SJIOException ioe) {

            System.err.println("[WorkerWest@{aboveNode}] Communication error: "+ioe);
