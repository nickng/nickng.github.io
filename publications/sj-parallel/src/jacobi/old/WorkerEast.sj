//$ sessionjc -cp lib -d lib src/jacobi/old/WorkerEast.sj
//$ sessionj  -cp lib:lib/jyaml.jar jacobi.old.WorkerEast mesh.yaml east
            ...
            try (left) {
                ...
                left.inwhile() {
                    ...
                    try (above, below) {
                        ...
                        System.err.println("[WorkerEast@{above,below}] Non-dual behaviour: "+ise);

                    } catch (SJIOException ioe) {

                        System.err.println("[WorkerEast@{above,below}] Communication error: "+ioe);

                    } catch (ClassNotFoundException cnfe) {

                        System.err.println("[WorkerEast@{above,below}] Class not found: "+cnfe);
                ...
                System.err.println("[WorkerEast@{left}] Non-dual behaviour: "+ise);

            } catch (SJIOException ioe) {

                System.err.println("[WorkerEast@{left}] Communication error: "+ioe);

            } catch (ClassNotFoundException cnfe) {

                System.err.println("[WorkerEast@{left}] Class not found: "+cnfe);

            } finally {} // Sockets

        } catch (SJIOException ioe) {

            System.err.println("[WorkerEast@{aboveNode,leftNode}] Communication error: "+ioe);
