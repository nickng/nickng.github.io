//$ sessionjc -cp lib/ -d lib/ src/jacobi/old/Master.sj
//$ sessionj  -cp lib:lib/jyaml.jar jacobi.old.Master mesh.yaml master 9 9 1.0 1.0 1.0 1.0
...
public class Master extends Jacobi {
    ...
    public void run(String rightHost, int rightPort, String belowHost, int belowPort, int nrOfNodes, int arraySize, double topBound, double rightBound, double downBound, double leftBound) {
        ...
        final noalias SJSocket right, below;
        ...
        try (right, below) {
            ...
            if (hasConverged(cc)) {
                ...
                Log.i("ver=Old arraySz="+arraySize, "End of calculation (reason: Master converged, iterations="+iterations+", Convergence parameter="+cc+").");

            } else if (iterations >= MAX_ITERATIONS) {

                Log.i("ver=Old arraySz="+arraySize, "End of calculation (reason: MAX_ITERATIONS("+MAX_ITERATIONS+") reached cc="+cc+").");

            }

        } catch (SJRuntimeException re) {

            System.err.println("[Master@{right,below}] Runtime error: "+re);

        } catch (SJIOException ioe) {

            System.err.println("[Master@{right,below}] Communication error: "+ioe);

        } catch (SJIncompatibleSessionException ise) {

            System.err.println("[Master@{right,below}] Non-dual behaviour: "+ise);

        } catch (ClassNotFoundException cnfe) {

            System.err.println("[Master@{right,below}] Class not found: "+cnfe);
        
        } finally {}

        Log.b("jacobi-ver=Old arraySz="+arraySize, ""+(System.currentTimeMillis() - time));
