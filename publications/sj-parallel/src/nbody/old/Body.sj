//$ sessionjc -cp lib:lib/jyaml.jar -d lib src/nbody/old/Body.sj
//$ sessionj  -cp lib:lib/jyaml.jar nbody.old.Body ring.yaml body
    ...
    public void run(int listenPort, String nextHost, int nextPort) throws ClassNotFoundException {
        ...
        try (prevNode) {
            ...
            try (prev,next) {
                ...
                Log.i("nbody.old.Body", "Connected to next node at "+nextHost+":"+nextPort);
                prev = prevNode.accept();
                Log.i("nbody.old.Body", "Connected by prev node at :"+listenPort);
